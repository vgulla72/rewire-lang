...
def find_jobs(input_data: PeopleSearchInput) -> PeopleSearchOutput:
    ...
    st.write(f"Searching for jobs: {', '.join(role_titles)}")
    for role in role_titles:
        # Build search query for this specific role
        query = (
            f'site: linkedin.com/jobs {role} AND {previous_title} AND {location}'
        )
